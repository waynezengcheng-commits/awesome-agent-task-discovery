...
type Task = (typeof tasks)[number] & { beneficiaryEvidence?: string[]; taskEvidence?: string[]; outcomeEvidence?: string[]; audioEvidence?: string[]; valueSignalScore?: number; technicalScore?: number; audioValueStatus?: string };
...
function ValueEvidence({ task }: { task: Task }) {
  ...
  const modalities = task.modalities?.join('、') || '文本';
  const taskText = `让 Agent 通过${surfaces}处理${modalities}信息，完成${task.taskEvidence?.[0] ? '一个可交付的自动化任务' : '一组待拆解的工作步骤'}。`;
  const valueText = task.outcomeEvidence?.length ? '公开描述提到效率、速度或质量改善；实际收益需要用真实流程复测。' : task.beneficiaryEvidence?.length ? '公开描述能看出服务对象，但还没有足够的结果数据证明收益。' : '目前只有技术线索，尚未补齐受益人和结果证据。';
