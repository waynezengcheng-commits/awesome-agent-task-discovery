...
export default function App() {
  ...
  return <>
    ...
    <main className="page" id="top">
      <section className="hero" aria-labelledby="hero-heading"><div className="hero-copy"><p className="eyebrow">SOURCE-BACKED TASK DISCOVERY</p><h1 id="hero-heading">先找到最值得交给 Agent 的工作。</h1><p>围绕高频、可执行、可衡量和可控风险四个信号，建立团队的 Agent 落地队列。</p><a className="text-link" href="#explore">查看任务机会 <ArrowUpRight size={15} /></a></div><div className="decision-canvas" aria-label="Agent 任务价值筛选方法"><div className="canvas-heading"><span>SIGNAL → PRIORITY</span><span className="mono">task.score()</span></div><div className="decision-flow"><div className="flow-in"><Layers size={20} /><span>工作信号</span></div><span className="connector" /><div className="fit-node"><Target size={23} /><strong>fit</strong></div><span className="connector" /><div className="flow-out"><span>频率</span><span>影响</span><span>边界</span></div></div><div className="canvas-footer">用可验证的任务闭环决定优先级。<ArrowRight size={14} /></div></div></section>
      <section className="stats" aria-label="任务统计"><div><span className="stat-value">{tasks.length.toString().padStart(2, "0")}</span><span>任务机会</span></div><div><span className="stat-value">{tasks.filter((task) => task.priority === "P0").length}</span><span>P0 优先验证</span></div><div><span className="stat-value">{categories.length - 1}</span><span>业务场景</span></div><div className="data-updated"><span>评分方法</span><strong>影响 · 频率 · 闭环</strong></div></section>
      <section className="explorer" id="explore"><aside className="sidebar"><div className="side-title">场景 <span>{categories.length - 1}</span></div><div className="category-list">{categories.map((item) => <button key={item} className={category === item ? "category active" : "category"} onClick={() => setCategory(item)}><Layers size={16} /><span>{item}</span><b>{item === "全部" ? tasks.length : tasks.filter((task) => task.category === item).length}</b></button>)}</div><div className="side-note"><span className="mini-radar"><ShieldCheck size={19} /></span><h3>从机会到试点</h3><p>先验证一个可控闭环，再扩大自动化范围。</p><a href={issueUrl()} target="_blank" rel="noreferrer">提交任务机会 <ArrowUpRight size={15} /></a></div><a className="side-source" href={repositoryUrl} target="_blank" rel="noreferrer">任务方法与数据 <ExternalLink size={12} /></a></aside>
