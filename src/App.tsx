...
export default function App() {
  ...
  const visible = useMemo(() => {
    ...
    return tasks.filter((task) => category === "全部" || task.category === category)
      ...
      .filter((task) => !onlyMultimodal || task.tags.includes("Multimodal"))
  ...
  }, [category, onlyMultimodal, onlySaved, priority, query, saved]);
  ...
  return <>
    <header className="header">
      ...
      <nav aria-label="主导航" className="header-utilities"><button className={!onlySaved ? "nav-item active" : "nav-item"} onClick={() => setOnlySaved(false)}>发现任务</button><button className={onlyMultimodal ? "nav-item active" : "nav-item"} onClick={() => setOnlyMultimodal((value) => !value)}><Sparkles size={16} /> 多模态 <span className="nav-count">{tasks.filter((task) => task.tags.includes("Multimodal")).length}</span></button><button className={onlySaved ? "nav-item active" : "nav-item"} onClick={() => setOnlySaved(true)}><Bookmark size={16} /> 收藏 <span className="nav-count">{saved.length}</span></button></nav>
    ...
    <main className="page" id="top">
      ...
      <section className="stats" aria-label="任务统计"><div><span className="stat-value">{tasks.length.toString().padStart(2, "0")}</span><span>任务机会</span></div><div><span className="stat-value">{tasks.filter((task) => task.priority === "P0").length}</span><span>P0 优先验证</span></div><div><span className="stat-value">{tasks.filter((task) => task.tags.includes("Multimodal")).length}</span><span>多模态候选</span></div><div className="data-updated"><span>公开发现</span><strong>每周更新</strong></div></section>
